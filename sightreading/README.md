# **Practice Sight-reading with Mic Support**

Sight-reading trainer webapp that supports keyboard input **and** listening  
to your instrument being played through a mic. Completely open source and  
runs locally in your browser. Check it out at  
[chausies.xyz/sightreading](https://chausies.xyz/sightreading).

## **Overview**

The webapp generates an infinite stream of random notes or chords on a  
virtual staff. Users read the notes and input it (via keyboard or  
mic+instrument) to advance. The app tracks accuracy and net speed for the  
session.

## **Features**

* **Infinite Stream of Notes:** Endless practice stream that dynamically  
  generates notes in various modes, including random and from chords.  
* **Supports Clefs and Keys:** Use any clef (including both bass and treble  
  aka grand staff) with any major/minor key, as well as 8va/8vb  
  transpositions.  
* **Microphone mode:** Listens on your mic for an instrument  
  being played. Great for use with a smart phone/tablet\!  
* **View Stats:** Check your accuracy and penalized notes per minute  
  (NPM).  
* **Teaching Mode:** Starts you off with a few notes, and dynamically  
  introduces new notes/bring back old notes based on your  
  performance.

## **Controls**

* **Pitch:** Type a through g  
* **Accidentals:** Type b for flat, or \# (or s) for sharp  
* **Octave:** Type a number (e.g., 4\) to submit your note guess (e.g.  
  c\#5)  
* **Settings:** Click the ⚙️ icon in the top right to configure your clef,  
  key signature, using mic+instrument, and more.

*Note: This project was largely vibecoded using Gemini.*
